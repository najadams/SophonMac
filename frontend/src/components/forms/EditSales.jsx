...
const EditSales = () => {
  ...
  const allowedUnits = company?.allowedUnits || [];
  const allowedCategories = company?.allowedCategories || [];
  ...
  const getInitialValues = () => {
    if (row) {
      ...
      const products = row?.detail?.map((product) => {
        const ceil = Math.ceil(product.salesPrice * (product.originalQuantity || product.quantity));
        return {
          name: product.name,
          quantity: product.originalQuantity || product.quantity, // Use original quantity if available
          unit: product.salesUnit || "none", // Use original sales unit
  ...
  const getProductUnits = (productName) => {
    if (!productName) return ["none"]; // Return default unit if product name is empty/null

    const product = productOptions.find(
      (p) => p.name.toLowerCase() === productName.toLowerCase()
    );
    if (!product) return ["none"]; // Default to none if product not found

    // Create array of available units including base unit and converted units
  ...
  const getUnitPrice = (productName, unit) => {
    ...
    console.log('getUnitPrice - Product:', product.name, 'Unit:', unit);
    console.log('getUnitPrice - Product conversions:', product.conversions);
    console.log('getUnitPrice - Base unit:', product.baseUnit, 'Base price:', product.salesPrice);

    // If it's the base unit, return the base sales price
    if (unit === product.baseUnit) {
      console.log('getUnitPrice - Using base unit price:', product.salesPrice);
    ...
    if (conversion) {
      console.log('getUnitPrice - Found conversion:', conversion, 'Price:', conversion.salesPrice);
      return conversion.salesPrice;
    }

    // If no conversion found, return the base price
    console.log('getUnitPrice - No conversion found, using base price:', product.salesPrice);
    return product.salesPrice;
  };
