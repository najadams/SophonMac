  ...
  const query = `
    SELECT * FROM (
      ...
      SELECT 
        ...
        r.createdAt as date,
        'Sale' as type,
        COALESCE(c.name || CASE WHEN c.company IS NOT NULL AND c.company != '' THEN ' (' || c.company || ')' ELSE '' END, 'Walk-in Customer') as reference,
      ...
      SELECT 
        ...
        c.name || CASE WHEN c.company IS NOT NULL AND c.company != '' THEN ' (' || c.company || ')' ELSE '' END as reference,
